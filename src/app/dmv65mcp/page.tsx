import { CheckCircle2, AlertTriangle, ShieldCheck, ArrowRight, FileText, Info, Search, ShieldAlert, BadgeCheck, ListChecks, Zap, XCircle, MapPin, Building2 } from "lucide-react";
import Link from "next/link";
import { DMV_SECTIONS, ContentSection, BROKERS_DATA } from "@/lib/data/brokers";
      ...
      {section.steps?.map((step, i) => (
        <div key={i} className="relative group">
          <div className="flex items-center mb-4">
            <div className="w-10 h-10 rounded-full bg-red-600 flex items-center justify-center text-white font-bold text-lg relative z-10 shadow-lg shadow-red-600/20 group-hover:scale-110 transition-transform">
              {i + 1}
            </div>
...
const ChecklistSection = ({ section }: { section: ContentSection }) => (
  <div className="bg-slate-900/40 border border-slate-800 rounded-[40px] p-10 mb-12">
    <h2 className="text-3xl font-bold text-white mb-8 flex items-center">
      <ListChecks className="w-8 h-8 mr-4 text-red-500" />
      {section.title}
    </h2>
  ...
  return (
    <main className="min-h-screen bg-[#0a0f1c] text-slate-200">
      ...
      <div className="relative overflow-hidden border-b border-slate-800 bg-slate-950/50 pt-24 pb-32">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-full bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-red-500/10 via-transparent to-transparent -z-10" />
        
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <Breadcrumbs 
            items={[
              { label: 'Rescue Funnels', href: '/' },
              { label: 'CA DMV MCP', href: '/dmv65mcp' }
            ]} 
          />
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
            <div className="lg:col-span-7">
              <div className="inline-flex items-center px-4 py-2 rounded-full bg-red-500/10 text-red-400 border border-red-500/20 mb-8 animate-pulse">
                ...
                Urgent: California Permit Suspension Warning
              </div>
              <h1 className="text-6xl md:text-8xl font-black tracking-tight text-white leading-[0.9] mb-8">
                California DMV <br/>
                <span className="text-red-500 font-italic">MCP-65 Filing</span>
              </h1>
              <p className="text-xl text-slate-400 leading-relaxed max-w-2xl">
                Is your California Motor Carrier Permit (MCP) suspended? We provide direct EDI filings to the Sacramento DMV database, restoring your authority in 24 hours or less.
              </p>
            </div>
            ...
            <div className="lg:col-span-5">
              <div className="bg-slate-900/50 backdrop-blur-xl p-8 rounded-[40px] border border-slate-800 shadow-2xl">
                <h3 className="text-xl font-bold text-white mb-6 flex items-center">
                  <BadgeCheck className="w-6 h-6 mr-2 text-red-500" />
                  Restore Your Permit Now
                </h3>
                <BrokerLeadForm brokerName="CA DMV MCP" ctaText="RESTORE MY PERMIT NOW" />
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="lg:col-span-8">
             ...
             <div className="bg-slate-900/40 border border-slate-800 rounded-[40px] p-10 backdrop-blur-sm mb-12">
              <h2 className="text-3xl font-bold text-white mb-10 flex items-center">
                ...
                California Filing Matrix
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {[
                  { label: 'Filing Type', val: 'MCP-65 (Electronic)' },
                  { label: 'Turnaround', val: '< 24 Hours' },
                  { label: 'DMV Database', base: 'EDI Direct Connection' },
                  { label: 'Authority', val: 'CA# & MCP Restore' },
                ].map((item, i) => (
                  <div key={i} className="p-8 bg-slate-950/50 rounded-3xl border border-slate-800 group hover:border-red-500/50 transition-all">
            ...
            {DMV_SECTIONS.map((section, index) => (
              <div key={index}>
                ...
                {section.type === 'callout' && (
                   <div className="bg-red-600/10 border border-red-500/20 rounded-[32px] p-8 mb-12 flex gap-6">
          ...
          <div className="lg:col-span-4">
            <div className="sticky top-8 space-y-8">
              ...
              <div className="bg-slate-900/40 border border-slate-800 rounded-[40px] p-10">
                ...
                <div className="p-6 bg-slate-950 rounded-2xl border border-slate-800 font-mono text-xs text-red-400 leading-relaxed">
                  California DMV Headquarters<br/>
                  Motor Carrier Permit Operations<br/>
                  PO Box 932370<br/>
                  Sacramento, CA 94232
                </div>
                <p className="mt-6 text-sm text-slate-500 leading-relaxed">
                  We maintain a direct digital link to the Sacramento MCP operations center, allowing us to bypass the standard mailroom delays.
                </p>
              </div>

              <div className="bg-gradient-to-br from-red-600 to-red-800 p-10 rounded-[40px] text-white shadow-2xl relative overflow-hidden">
                <h3 className="text-2xl font-bold mb-4 relative z-10">CHP OOS Status?</h3>
                <p className="text-red-100 text-sm mb-8 relative z-10">
                  If you are currently pulled over by the CHP, call us now. We can push an emergency filing in minutes.
                </p>
                <Link href="/contact" className="bg-white text-red-700 font-bold px-8 py-4 rounded-2xl inline-block relative z-10 hover:bg-slate-100 transition-all w-full text-center">
                  EMERGENCY FILING
                </Link>
              </div>
            </div>
