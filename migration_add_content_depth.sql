-- MIGRATION: ADD CONTENT DEPTH (Schema + Data)
...
-- Add content_markdown to Violations
DO $$ 
BEGIN 
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'violations' AND column_name = 'content_markdown') THEN 
        ALTER TABLE violations ADD COLUMN content_markdown TEXT; 
    END IF; 
END $$;

-- Add content_markdown to State Filings
DO $$ 
BEGIN 
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'state_filings' AND column_name = 'content_markdown') THEN 
        ALTER TABLE state_filings ADD COLUMN content_markdown TEXT; 
    END IF; 
END $$;

-- Add content_markdown to Trailer Profiles
DO $$ 
BEGIN 
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'trailer_risk_profiles' AND column_name = 'content_markdown') THEN 
        ALTER TABLE trailer_risk_profiles ADD COLUMN content_markdown TEXT; 
    END IF; 
END $$;


-- =========================================================================
-- 2. DATA SEEDING (The "Thickening")
-- =========================================================================

-- 2.1: 395.8(e) False Logs
UPDATE violations 
...
2.  **Personal Conveyance (PC) Misuse:** Using PC to advance a load (even 1 mile).
...
-- 2.2: 382.215 Controlled Substances (Drug Test)
UPDATE violations 
SET content_markdown = E'
...
-- 2.3: 395.8(a) No Record of Duty Status (No Logs)
UPDATE violations 
SET content_markdown = E'
...
'
WHERE slug = 'hot-shot-insurance'; -- Verify slugs match seed_trailers.ts
